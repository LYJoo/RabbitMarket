...
<script>
    ...
    function listCall(reqPage){	//페이지 요청 함수
        var reqUrl ='./blackList/'+pagePerNum+"/"+reqPage; //restful 로 요청 -> /list/보여줄갯수/페이지
        $.ajax({
            url:reqUrl
            ,type:'get'
            ,data:{}
    ...
    function listPrint(list){
        ...
        for(var i=0;i<list.length;i++){
            ...
            content +="<td>"+list[i].black_reason+"</td>";
            var reg_date = new Date(list[i].reg_date);
            content +="<td>"+reg_date.toLocaleDateString("ko-KR")+"</td>";
            content +="<td>"+list[i].admin_id+"</td>";
            content +="<td>"+list[i].unblack_reason+"</td>";
            if(list[i].unblack_reason != ""){
            	content += "<td><a onclick='openUnBlackManage(this)' id='"+list[i].member_id+"' class='"+list[i].admin_id+"' style='cursor:pointer;'>해제</a></td>";
            }else{
            	content += "<td>"+list[i].unblack_date+"</td>";            	
            }
            content +="</tr>";
        }
